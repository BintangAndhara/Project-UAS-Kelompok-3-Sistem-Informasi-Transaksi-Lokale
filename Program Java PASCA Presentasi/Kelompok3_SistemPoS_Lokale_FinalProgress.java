...
// [Revisi Dosen: Class tambahan untuk Encapsulation password user agar bersifat private]
class Akun {
...
class Transaksi {
    ...
    public void tambahProduk(Produk p, int jumlah) {
        // [Revisi Dosen: Cek apakah produk sudah ada di keranjang untuk digabungkan agar hemat baris struk]
        for (int i = 0; i < this.jumlahItem; i++) {
            if (this.keranjang[i].getKodeProduk().equals(p.getKodeProduk())) {
                this.qty[i] += jumlah; // Tambahkan qty ke produk yang sudah ada
                return; // Keluar dari method agar tidak membuat baris baru
            }
        }

        // Jika produk belum ada, tambahkan sebagai baris baru
...
public class Kelompok3_SistemPoS_Lokale_FinalProgress {
    public static void main(String[] args) {
        ...
        // Inisialisasi Akun untuk Fitur Login (Revisi Dosen)
        Akun akunKasir = new Akun("kasir", "123");
        Akun akunAdmin = new Akun("admin", "123");
        ...
        while (programJalan) {
            try {
                ...
                if (!isLoggedIn) {
                    ...
                    // [Revisi Dosen: Hide password input menggunakan Console]
                    ...
                    if (console != null) {
                        ...
                        // Fallback jika program di run di terminal IDE yang tidak support Console
                        inputPassword = input.nextLine();
                    }

                    // Menggunakan method cekPassword dari object Akun
                    if (inputUsername.equals(akunKasir.getUsername()) && akunKasir.cekPassword(inputPassword)) {
                        roleAktif = "Kasir";
                        isLoggedIn = true;
                        System.out.println("> Login Berhasil! Akses Kasir Diberikan.\n");
                    } else if (inputUsername.equals(akunAdmin.getUsername()) && akunAdmin.cekPassword(inputPassword)) {
                        roleAktif = "Admin";
                        isLoggedIn = true;
                ...
                // [Revisi Dosen: Penyesuaian opsi menu Kasir (1-3) dan Admin (1-2)]
                if (roleAktif.equals("Kasir")) {
                    System.out.println("1. Lihat Daftar Menu");
                    System.out.println("2. Buat Transaksi Baru");
                    System.out.println("3. Logout");
                } else if (roleAktif.equals("Admin")) {
                    System.out.println("1. Tutup Shift & Cetak Laporan");
                    System.out.println("2. Logout");
                }
                
                System.out.print("Pilih menu: ");
                int pilihanMenu = Integer.parseInt(input.nextLine()); 

                // Mapping input angka ke case switch agar struktur case tetap utuh
                int switchTarget = -1;
                if (roleAktif.equals("Kasir")) {
                    if (pilihanMenu == 1) switchTarget = 1;
                    else if (pilihanMenu == 2) switchTarget = 2;
                    else if (pilihanMenu == 3) switchTarget = 4;
                } else if (roleAktif.equals("Admin")) {
                    if (pilihanMenu == 1) switchTarget = 3;
                    else if (pilihanMenu == 2) switchTarget = 4;
                }
                    ...
                    case 2:
                        ...
                        while (true) {
                            try {
                                ...
                                idKasir = Integer.parseInt(input.nextLine());
                                if (idKasir >= 0 && idKasir <= daftarKasir.length) {
                                    break;
                                }
                                System.out.println("> Pilihan kasir tidak valid! Silakan masukkan angka 1-" + daftarKasir.length + " atau 0.\n");
                            } catch (Exception e) {
                            ...
                            if (foundIndex != -1) {
                                ...
                                while (true) {
                                    try {
                                        ...
                                        System.out.println("> Input tidak valid! Silakan input qty > 0.\n");
                                    } catch (Exception e) {
                                        System.out.println("> Input tidak valid! Harap masukkan angka.\n");
                                    }
                                }
                        ...
                        while (true) {
                            try {
                                ...
                                trx.metodeBayar = Integer.parseInt(input.nextLine());
                                if (trx.metodeBayar >= 1 && trx.metodeBayar <= 3) {
                                    break;
                                }
                                System.out.println("> Metode bayar tidak valid! Silakan pilih 1, 2, atau 3.\n");
                            } catch (Exception e) {
                                System.out.println("> Input tidak valid! Harap masukkan angka.\n");
                            }
                        }
